...
- **Flow metodu:** `step8_code_review` (`flow.py:2859`) + `_review_retry_loop` (`flow.py:729`)
- **Ajan:** `code_reviewer`
...
- env/knob: `CREW_REVIEW_MAX_RETRIES` (default 2), `CREW_SM_REVIEW`

## İşleyiş
1. **Dry-run** (KN-23): review atlanır.
2. **PR yorumlarına yanıt** (`flow.py:2872`): resolve edilmemiş insan yorumları —
   dosya düzeltildiyse "Düzeltildi" + resolve; plan dışıysa açıklama; genel
   yorumsa plan özeti.
3. **Kod inceleme** — HAL dalı `self._hal.followup`, CrewAI dalı `create_review_crew`.
4. **SM Review** (KN-09).
5. **Verdict kontrolü** (KN-30, `flow.py:2996`): çıktıda `CHANGES_REQUIRED` /
   `REJECTED` / Türkçe eşdeğerleri aranır.
6. **Retry döngüsü** (KN-31, `_review_retry_loop`): RED ise reviewer'ın bahsettiği
   dosyalar yeniden implement edilip push edilir, tekrar review yapılır. Maks.
   `CREW_REVIEW_MAX_RETRIES` deneme; aşılırsa WI'ya hata yorumu + pipeline durur.
...
- **KN-31** — Review retry döngüsü + max retry. Bkz. [decision-points.md#kn-31](../decision-points.md#kn-31)
...
## Kaynak
- `flow.py:2859-3044` (`step8_code_review`), `flow.py:729-880` (`_review_retry_loop`)
- `tasks.yaml:636-736` (`review_pr_task`)
