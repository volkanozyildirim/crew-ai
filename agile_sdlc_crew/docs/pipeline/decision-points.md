...
## KN-48 — Ajan becerileri: CrewAI Agent Skills ve `code-review`
- **Nerede:** `skills/` paketi (`skills/code-review/SKILL.md` + `references/{php,go,typescript,python,sonarqube}.md`) · `crew.code_reviewer` (`skills=[...]`) · `flow._review_skill_context` → `skills.review_context` · `pr_review.build_pr_context` · knob `CREW_AGENT_SKILLS`.
- **Karar:** İnceleyici ajan neye göre inceleyecek; bu bilgi prompt'a nasıl girecek?
- **Girdi:** `CREW_AGENT_SKILLS` (açık) · CrewAI'ın native Agent Skills desteği (`Agent(skills=[...])`, SKILL.md frontmatter, kademeli açılım) · repo dili `knowledge.detect_repo_type` (composer.json → php, go.mod → go, package.json → nextjs/react/js, pyproject → python) · klonlu repolardaki gerçek `sonar-project.properties` (`sonar.qualitygate.wait=true`, 7 repo).
- **Sonuç:** `code-review` skill'i **RESOURCES (3)** seviyesinde bağlanır: gövde (kapsam disiplini, SOLID, güvenlik, Sonar politikası) her incelemede sistem prompt'una girer, `references/` kataloğu da listelenir. **Derinlik pipeline tarafından enjekte edilir**: repo diline uyan TEK referans + `sonarqube.md` review context'ine eklenir (PHP repo → ~7.5 KB; dil çözülemezse yalnızca Sonar). Ret sebepleri üçle sınırlı — **eksik / bozuk / fazla**; bunun dışındaki her şey `minor`, yani yorum. Sonar kuralı **Clean as You Code**: yalnızca PR'ın dokunduğu satırlardaki ihlal blocker, mevcut borç bu PR'ın meselesi değil. Yeni kod coverage'ı `minor` (test zorunluluğu ayrı knob: `CREW_REQUIRE_TESTS`). Dil referansı **öneri kaynağıdır** — "daha modern yazılabilirdi" tek başına blocker değildir.
- **Neden:** Kullanıcı: "code review adımı için bir skill yükle… review kapsamı work item talebiyle sınırlı kalsın… dilin en güncel kullanımına göre yorumlasın, SOLID ve güvenlik… SonarQube'da borç oluşturmamalı." Sonar prod'da `qualitygate.wait=true` ile build'i bloke ettiği için buradan geçen ihlal orada patlıyordu. Clean as You Code seçildi çünkü "yeni borç oluşmasın" ile "kapsam WI ile sınırlı" isteklerini aynı anda karşılayan tek model o. Referansın ajan tarafından değil pipeline tarafından açılmasının sebebi: `code_reviewer` claude_cli üzerinde koşuyor ve CrewAI'ın Python araç döngüsü o sürece köprü kuramıyor (KN "claude_cli tool bridge"); repo dili zaten deterministik olarak biliniyor, dolayısıyla doğru referansı enjekte etmek hem daha ucuz hem daha güvenilir.
