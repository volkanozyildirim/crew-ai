...
- **Uygulanabilir öneri (suggestion):** Satır yorumu, düzeltme **mekanik ve satır-yerel** ise Azure DevOps'un ```suggestion bloğunu taşır — PR sahibine "Apply changes" butonu çıkar. Reviewer `suggested_code` + `suggested_line_start/end` doldurur (sözleşme `review_pr_task`'ta, kural `code-review` skill'inde); yapısal düzeltmelerde doldurmaz. `pr_review.verified_suggestion` öneriyi **dosyayı yeniden okuyarak** doğrular: dosya tanınıyor mu, aralık dosyanın içinde mi, öneri zaten oradaki kodla aynı mı. Doğrulanmayan öneri düşer ve yorum düz metne döner — bulgu kaybolmaz. Thread aralığı (`add_pr_comment(end_line=...)`) değiştirilecek satırlarla birebir örtüşür, aksi hâlde "Apply changes" yanlış yeri günceller. Yalnızca 🔍 insan PR incelemesinde; pipeline'ın kendi review adımında düzeltmeyi zaten geliştirici ajan yapıyor.
- **Neden:** Kullanıcı: "ilerlemiş işlere müdahale edemiyoruz ama review sürecine katkı sağlayacak bir alan açalım." Suggestion için: "review sonucu öneriler PR'a suggestion olarak eklensin." Doğrulama katmanının sebebi: öneri tek tıkla commit'lenebiliyor, yanlış satıra yazılan bir öneri sessizce kod bozar. Sahiplik ilkesi (KN-35/41) korunur; katkı yalnızca review sürecine, insan tetikli ve geri alınabilir (yorum).
...
- **Girdi:** `CREW_BACKLOG_REFINEMENT` (açık) · kapsam: seçili sprint (`IterationPath =`) ya da takımın alan yolu altındaki tüm Proposed işler (`AreaPath UNDER`, `get_team_area_path`) · **tip seti kapsama göre** (`scope_types`): sprintte yalnızca çocuk işler — `CREW_REFINEMENT_SPRINT_TYPES`, varsayılan `Task,Bug,Improvement` (boş/bozuk değer varsayılana düşer); backlog'da tüm tipler (User Story / PBI / Bug / Task / Improvement) · Proposed durum adları **o kapsamın tiplerinin** süreç listesinden (`category == Proposed`), yoksa ad tabanlı yedek · WI alanları: açıklama, kabul kriteri, repro adımları, SP (Custom.StoryPoints → StoryPoints → Effort), öncelik, etiket, oluşturma tarihi, ebeveyn ilişkisi · eşik `CREW_REFINEMENT_MIN_SCORE` (70), bayatlama `CREW_REFINEMENT_STALE_DAYS` (30), kısa açıklama eşiği `CREW_MIN_WI_CONTENT_CHARS`.
...
- **Neden:** Hazırlık skoru yalnızca pipeline içinde ve ücretli BA çağrısından sonra hesaplanıyordu (KN-30); sprint planlama adayları sıralıyor ama hazırlığa bakmıyordu. Sprint kapsamındaki tip filtresi sonradan eklendi (kullanıcı: "sprintte refinementa sadece tasklar girecek, user storyleri hariç tut"): sekme sprintte yalnızca sprint'e atanmış **ebeveyn** User Story'leri gösteriyordu — aktif sprintte 4 satırın dördü de buydu, gelecek sprintte 24 satırın 4'ü — yani board'un gösterdiği çocuk işlerle hiç örtüşmüyordu. Refinement töreni için sıfır maliyetli, deterministik bir ön kontrol gerekiyordu; LLM ve yazma insan kararıyla açılır.

## KN-47 — Yaşlandırma: "iş kaç gündür bu durumda?" (salt görsel)
- **Nerede:** pipeline dışı · `aging.py` (`bucket_of`, `classify`, `annotate`, `thresholds`) · `GET /api/board/workitems` (her işe `aging` bloğu), `GET /api/aging/config`, `GET /api/refinement` (satırda `aging` + `state_days`) · dashboard: board kartı gün rozeti/kenar rengi, kolon başlığı sayacı, sprint özetinde "Bekleyen" filtresi, Genel bakış **Takılan işler** kartı, Refinement tablosunda **Bekleme** kolonu + "Bekleyen" filtresi.
- **Karar:** Bir iş şu anki durumunda ne kadar süredir kımıldamıyor ve bu, o durum için normal mi?
- **Girdi:** `CREW_AGING` (açık) · süre = şimdi − `Microsoft.VSTS.Common.StateChangeDate`; alan boşsa `System.ChangedDate`, o da yoksa `System.CreatedDate` (hangisinin kullanıldığı `source` ile tooltip'e yazılır) · durum adı → dört kova: **backlog** (New/Backlog/Proposed/Approved), **todo** (To Do/Committed/Ready/…), **progress** (Active/In Progress/Code Review/QA/Testing/UAT/Preprod Check/…), **blocked** (Blocked/On Hold/Waiting) · `DONE_STATES` → **hiç yaşlanmaz**: Done/Closed/Completed/Removed/Canceled/Rejected/Duplicate/Released **ve bu org'da tamamlanmış sayılan Resolved + Ready for Production** (`sprint_report.DONE_STATES` ve dashboard `DONE_STATES` ile aynı liste — ayrışırsa board 'tamamlandı' dediği işi aynı anda 'takılı' diye kırmızıya boyardı); ayrıca `DONE_RE` ile 'Closed - Won't Fix' gibi türevler, tanınmayan özel durum → `todo` kovası · eşikler kova başına `CREW_AGING_<KOVA>_WARN_DAYS` / `_CRIT_DAYS` (varsayılan sıkı: backlog 14/45, todo 3/7, progress 2/4, blocked 1/3), `pipeline_config` üzerinden dashboard'dan da düzenlenir.
- **Sonuç:** Her işe `{days, level: ok|warn|crit, bucket, warn, crit, source, since, text}`. Eşiği aşan board kartı sol kenar rengi + `Ng` rozeti (kritikte hafif zemin), kolon başlığında sayaç, kolon içinde kritik → uyarı → normal sıralaması (eşit seviyede Azure öncelik sırası korunur), sprint özetinde tıklanabilir "Bekleyen N" filtresi, Genel bakış'ta en uzun bekleyen 8 iş. Refinement'ta bayatlık cezası kademeli: `stale` −5 (`CREW_REFINEMENT_STALE_DAYS`, 30) ve backlog kritik eşiğini (45g) aşanlarda `very_stale` −15; ayrıca ölçü artık **oluşturma yaşı değil durumda bekleme** — sprint'e taşınıp durumu değişen eski iş yeniden taze sayılır (satır alt bilgisinde yaş "N günlük" olarak ayrıca durur). **Hiçbir yazma yok:** WI'a yorum/alan yazmaz, durum değiştirmez, iş kuyruğa almaz, LLM çağırmaz.
- **Neden:** Kullanıcı: "işler belirli statülerde uzun süre kalırsa, örneğin todo, in progress gibi ya da backlog, bu durumu ön plana çıkaracak arayüz düzenlemelerini yap" — ardından "refinement'a girdim tüm backlog seçtim 1 yıldır bekleyen işler var". Tek tek bakınca normal görünen kartlar, "kaç gündür orada" yazmadıkça hiç göze çarpmıyordu; skor tarafında da 1 yıllık iş ile 31 günlük iş aynı −5 cezayı alıyordu. Eşikler sunucuda çözülür (arayüz eşik hesaplamaz) ki env/dashboard değişikliği HTML'e dokunmadan yansısın.

## KN-48 — Ajan becerileri: CrewAI Agent Skills, tüm pipeline
- **Nerede:** `skills/` paketi (9 skill) · `crew.py` tüm ajan kurucuları (`skill_kwargs`) · `flow._standards_context` → `skills.standards_context` (tasarım/geliştirme/inceleme adımları) · `pr_review.build_pr_context` · knob `CREW_AGENT_SKILLS`.
- **Karar:** Ajanlara "nasıl karar verecekleri" nasıl öğretilir; bu bilgi prompt'a nasıl girer?
- **Girdi:** `CREW_AGENT_SKILLS` (açık) · CrewAI'ın native Agent Skills desteği (`Agent(skills=[...])`, SKILL.md frontmatter, kademeli açılım) · repo dili `knowledge.detect_repo_type` · klonlu repolardaki gerçek `sonar-project.properties` (`sonar.qualitygate.wait=true`, 7 repo).
- **Sonuç:** Rol başına bir skill — `agile-facilitation` (SM), `requirements-analysis` (BA), `technical-design` (mimar), `implementation` (geliştirici), `code-review` (inceleyici), `test-planning` (QA), `uat` (UAT), `product-assessment` (PO) — artı üç rolün paylaştığı `engineering-standards` (mimar + geliştirici + inceleyici). Skill'ler **RESOURCES (3)** seviyesinde bağlanır: gövde + `references/` kataloğu sistem prompt'una girer. Dil derinliği **pipeline tarafından** enjekte edilir: `standards_context(repo)` repoya uyan TEK dil referansı + `sonarqube.md` üretir ve bunu `technical_design_task`, `implement_change_task`, `review_pr_task` bağlamına ekler (PHP repo → ~7,5 KB; dil çözülemezse yalnızca Sonar). İnceleme kararı üç kapıyla sınırlı — **eksik / bozuk / fazla**; gerisi `minor`. Sonar kuralı **Clean as You Code**: yalnızca dokunulan satırlardaki ihlal blocker. `knowledge/` mekanizması **kaldırılmadı**: knowledge = alan bilgisi (FLO stack nasıl çalışır), skill = karar kuralları + kademeli açılan referans; ikisi birlikte bağlanıyor.
- **Neden:** Kullanıcı: "code review adımı için bir skill yükle… kapsam work item talebiyle sınırlı kalsın… dilin en güncel kullanımına göre yorumlasın, SOLID ve güvenlik… SonarQube'da borç oluşturmamalı" ve ardından "tüm pipeline için skill ve agent yapısını ayarla". `engineering-standards`'ın mimar ve geliştiriciye de verilmesinin sebebi maliyet: incelemede yakalanan her ihlal bir düzeltme turu, yani ek LLM çağrısı ve süre; aynı ölçütü yazan tarafa vermek hatayı en ucuz yerde yakalar. Referansın ajan tarafından değil pipeline tarafından açılmasının sebebi: ajanlar claude_cli üzerinde koşuyor ve CrewAI'ın Python araç döngüsü o sürece köprü kuramıyor (KN "claude_cli tool bridge"); repo dili zaten deterministik biliniyor. **Tuzak:** `skill_kwargs` CrewAI'a önceden yüklenmiş `Skill` nesneleri verir, dizin yolu DEĞİL — `load_skills` bir `Path`'i arama dizini sayıp altındaki tüm skill'leri yükler, yani `skills/` yolunu geçmek her ajana dokuz skill'i birden bağlardı. **İkinci tuzak:** frontmatter `description` değerinde `: ` varsa YAML kırılır; değerler tırnaklanmalı (dokuz skill'in ikisi bu yüzden sessizce yüklenmedi, `skill_kwargs` uyarı yazıp atladı).
