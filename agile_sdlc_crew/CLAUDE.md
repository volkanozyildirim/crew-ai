...
- Kickoff (step0): the active `run_kickoff_meeting` path uses `_build_kickoff_agents` → `build_for_agent(...)` → dashboard `config/agent_llm_overrides.yaml`. It is **NOT** local Ollama. (The legacy `create_kickoff_crew` is unused.) Its architect and developer bind to the *kickoff* keys below, not to the pipeline ones.
- **Grunt vs decision split** — an agent's cheap phases have their own `agent_key`, so the model is chosen per *phase*, not per persona: `software_architect_explore` (Faz A repo scan), `software_architect_kickoff`, `senior_developer_kickoff` → `architect_explore_cli` / `kickoff_cli` (sonnet) via `agent_defaults`. The persona still comes from `software_architect` / `senior_developer` in `agents.yaml` — only the LLM differs. **The decision points stay premium and must not be moved onto these keys**: `_architect_emit_json` and `_amend_plan` write the plan, `create_code_crew` writes the code, `code_reviewer` is the gate. `create_analysis_crew_toolless(agent_key=...)` defaults to `software_architect`, so any call that does not opt in keeps the old behaviour. The keys are deliberately absent from `agent_llm_overrides.yaml` (that file is the human/dashboard layer) and are excluded from the `CREW_USE_LOCAL_LLM` fallback in `_backwards_compat_profile` — exploration is 15+ turns of native `Read`/`Grep`/`Glob` tool use, which a local 8B model does badly and would silently return an empty scan. They are registered in `server._AGENT_KEYS` and in `assert_models_reachable` because unregistered config drifts silently. Rollback is one env var: `CREW_LLM_PROFILE_SOFTWARE_ARCHITECT_EXPLORE=architect_cli`. Measured basis: exploration was 29 calls / **43%** of `technical_design_task` ($34.40 of $80.60, $1.19 per call).
- **Why exploration is expensive — turns, not context.** A row in `llm_calls` is one `claude -p` *session*, so its token counts are summed over every turn. The 29 exploration calls: fresh input 161K (0.8%), cache **write** 1.42M (7.2%), cache **read** 18.2M (**92%**), across 450 turns (15.5 per call). That is **~44K per turn, ~40K of it cache read** — the real per-request context is ordinary, nowhere near any model's window. The cost is a ~44K prefix re-read on all 15.5 turns. So the levers are **fewer turns** or **a smaller prefix**, not a bigger-context model — and the same arithmetic explains the 3.8 cache read/write ratio the Maliyet tab reports.
...
- **WI lifecycle & DoD** (`wi_lifecycle.py`, `CREW_WI_LIFECYCLE` default **off**, `CREW_DOD_CHECKLIST` default on): the pipeline may move the WI's `System.State` (start → In Progress, PR → Code Review, needs_info/needs_human → Blocked, DoD passed → QA To Do/QA). State names are **not** hardcoded — `STATE_PREFS` picks the first match from the type's `workitemtypes/{type}/states`; transitions happen only inside the *owned range* (Proposed category + In Progress/Code Review/Blocked), never on QA/UAT/Done. DoD is deterministic (review approve, open issues, `state.build_status`, UAT PASS/FAIL, test paths, PR); `CREW_DOD_ENFORCE` turns a failed DoD into `needs_human`. See KN-35..37.
- **Estimation & child tasks** (`estimation.py`, `wi_children.py`; `CREW_ESTIMATE` on, `CREW_WI_WRITE_ESTIMATE` / `CREW_WI_CHILD_TASKS` off): SP = max(BA `estimate` block, structural signals) snapped to Fibonacci, only ever rises; written to `jobs.estimate_sp` and the completion comment ("Tahmin … · Gerçekleşen …"). StoryPoints is written to the WI only when empty, to `Custom.StoryPoints` first (the org's real field; board/sprint report/velocity read it), then the template fields. Child Tasks are created only for parent-type WIs (User Story/Bug/Feature), tagged `crew-generated`, idempotent, closed per pushed file. See KN-38/39.
- **Retrospective** (`retrospective.py`, `GET /api/retro`, dashboard 🔁 Retro, `CREW_RETRO` on): deterministic sprint/last-N-days learning report from `jobs`/`job_steps` (outcome classes via `classify_outcome`, review retries, build/UAT gates, SP per minute/$). Rule suggestions are threshold-based and only enter `kickoff_guidance` when a human clicks "Ekle" — never auto-added. See KN-40.
- **Sprint planning & daily** (`sprint_planning.py`, `daily.py`; `CREW_SPRINT_PLANNING` on, `CREW_DAILY_ENABLED` off): 🗓️ builds a read-only candidate list from the selected sprint (Proposed state, pipeline type, no open/completed job) ordered by priority and SP, pre-selected up to capacity (user or team velocity), with retro-based cost/time; queueing happens only via the confirm button and skips WIs with open jobs. ☀️ renders the last-N-hours summary (finished/running/queued/waiting-on-human) from `jobs`; the scheduler posts it daily to `CREW_DAILY_DIR` and, if `CREW_DAILY_TELEGRAM_TOKEN/CHAT_ID` are set, Telegram. See KN-41/42.
- **Type-based flow & PO** (`type_flow.py`; `CREW_TYPE_FLOW` on, `CREW_PO_ASSESSMENT` off): `state.flow_kind` (bug/story/task/spike) is set in `_wi_begin`; Bug/Story add an English guidance block to every step context (appended to `parts`, so the cache prefix stays stable) and Bug makes the DoD test item mandatory. A spike (explicit type/tag/title marker only) skips planning and code: one architect exploration → Turkish report on the WI → remaining steps "Atlandı" → `_SpikeStop` (subclass of `_KickoffOnlyStop`, job completed). The optional PO agent runs after the readiness gate, is advisory only (HOLD never stops the pipeline). See KN-43/44.
- **Backlog refinement** (`backlog_refinement.py`, `GET /api/refinement`, `POST /api/refinement/action`, dashboard Refinement tab; `CREW_BACKLOG_REFINEMENT` on): LLM-free Definition of Ready for Proposed-state WIs. **Scope decides the work-item types**: the *selected sprint* checks only child work (`CREW_REFINEMENT_SPRINT_TYPES`, default `Task,Bug,Improvement`) because a sprint's parent User Stories are not what gets refined there — this matches the board, whose own WIQL is `WorkItemType <> 'User Story'`; *whole team backlog* (via `AreaPath UNDER`) keeps every type, since story-level DoR belongs there. `scope_types()` resolves it and the Proposed state names are read from that same type set. Penalties for missing description/AC (Bug: repro; spike: research question), SP, priority, parent, SP>8, short title, staleness → score 0-100 vs `CREW_REFINEMENT_MIN_SCORE` (70); structural SP suggestion. Write actions (`CREW_REFINEMENT_WRITE` off): per-row human click → gaps comment, SP write (never overwrites), `needs-refinement` tag. `CREW_REFINEMENT_LLM` (off): BA agent (toolless, `refinement_questions_task`) drafts clarifying questions + AC; jobs carry `job_kind=refinement`. Never changes state or queues jobs. See KN-46.
- **Aging / stuck work** (`aging.py`, `GET /api/aging/config`; `CREW_AGING` on): how long a WI has sat in its *current* state — `Microsoft.VSTS.Common.StateChangeDate` → `ChangedDate` → `CreatedDate`. States map to four buckets (backlog / todo / progress / blocked) with per-bucket warn+crit day thresholds (`CREW_AGING_<BUCKET>_WARN_DAYS` / `_CRIT_DAYS`, registered in `pipeline_config` so the dashboard can edit them); `DONE_STATES` never age — including **Resolved** and **Ready for Production**, which this org counts as complete (kept in sync with `sprint_report.DONE_STATES` and the dashboard's `DONE_STATES`; diverging makes the board paint a "completed" item red as stuck) — unknown states fall into `todo`. The server annotates `/api/board/workitems` and `/api/refinement` rows with an `aging` block — **the UI never computes thresholds, it only colors**. Dashboard surfaces: board card day badge + left border, kanban column counter, stuck-first ordering inside a column, "Bekleyen" filter in the sprint summary, Genel bakış "Takılan işler" card, Refinement "Bekleme" column + filter. In refinement the staleness penalty is now tiered — `stale` −5 (still gated by `CREW_REFINEMENT_STALE_DAYS`) vs `very_stale` −15 past the backlog crit threshold — and it measures time-in-state, not creation age (`age_days` is still reported separately). Read-only: no WI writes, no state changes, no LLM. See KN-47.
- **Agent skills** (`skills/` package, `CREW_AGENT_SKILLS` on): CrewAI's native Agent Skills (`Agent(skills=[...])`, `SKILL.md` frontmatter, progressive disclosure) — one skill per role, wired in every agent builder in `crew.py`: `agile-facilitation` (SM), `requirements-analysis` (BA), `technical-design` (architect), `implementation` (developer), `code-review` (reviewer), `test-planning` (QA), `uat`, `product-assessment` (PO), plus `engineering-standards` shared by architect + developer + reviewer. This does **not** replace `knowledge/` — knowledge is domain background (how the FLO stack works, injected whole), skills are decision rules plus references opened on demand; both stay wired. Two traps: (1) `skill_kwargs` passes **pre-loaded Skill objects**, never a directory path — CrewAI treats a `Path` as a *search dir* and would attach all nine skills to every agent; (2) a frontmatter `description` containing `: ` breaks YAML, so values must be quoted (two skills silently failed to load before this was fixed — `skill_kwargs` logs and skips). Agents can't open reference files themselves (they run on claude_cli, where the CrewAI tool bridge is broken), so `skills.standards_context(repo)` resolves the repo language via `knowledge.detect_repo_type` and injects the ONE matching reference + `sonarqube.md` into the **technical design, implementation and review** step contexts (and `pr_review.build_pr_context`) — giving the writer the same yardstick as the reviewer, because a violation caught at review costs a whole retry round. `code-review` binds rejection to three gates — **missing / broken / extra** vs the work item — everything else is `minor`. Sonar follows **Clean as You Code**: only violations on lines the change touched block (repos run `sonar.qualitygate.wait=true`, so the gate fails their build); pre-existing debt is out of scope and new-code coverage is a `minor` warning because `CREW_REQUIRE_TESTS` owns that. See KN-48.
- **Cost analytics** (`cost_analytics.py`, `GET /api/costs`, dashboard **Maliyet** tab; `CREW_COST_ANALYTICS` on): reads `llm_calls` and breaks spend down by step / model / **model family** / agent / job_kind / job, plus the token mix and threshold-based findings. The data was already being written by the `claude_cli_llm` sink — until now the only reader was `db.get_job_cost_summary(job_id)` (one job), so any cross-job question meant hand-written SQL. Read-only: no writes, no LLM, zero cost. Two traps the thresholds encode: (1) `BY_DESIGN_MULTI` (`kickoff_meeting_task`, `pr_build_gate`) are multi-call **by design**, so a high calls-per-job there is reported as info, not a loop warning; (2) `usd_per_mtok` must **not** be used to compare model prices — cache-read tokens bill at a tenth of input, so a cache-heavy model looks cheap; class comparison goes through `by_family` share instead. Grouping happens in Python because this MySQL rejects `LIMIT` inside an `IN` subquery and reserved words like `out` as aliases. **One `llm_calls` row is one `claude -p` subprocess (multi-turn), not one API request** — token sums are per session, divide by `turns` for per-request context. Baseline at first run (601 calls / 43 jobs / $209): 68% of spend in `technical_design_task` + `review_pr_task`, 97% on the opus class, cache read/write ratio 3.8. Dashboard renders the server's markdown through `mdToHtml`, so thresholds and formatting live in Python only.
- **PR review contribution** (`pr_review.py`, `POST /api/pr-review`, board 🔍 on started WIs; `CREW_PR_REVIEW` on): advisory review of a human-developed PR — resolves the active PR from WI relations, pre-fetches changed files, runs `code_reviewer` once, posts a summary + inline blocker/major comments on the PR and a WI comment. Never changes code, votes, or WI state. Jobs carry `job_kind=pr_review`; retro/sprint-plan/daily queries filter `job_kind=pipeline`. Inline comments carry an Azure DevOps ```suggestion block (one-click "Apply changes") when the fix is mechanical and line-local — the reviewer fills `suggested_code` + `suggested_line_start/end`, and `verified_suggestion` re-reads the file and drops the suggestion unless the range is inside it and actually differs from what's there; a dropped suggestion falls back to prose. The thread range must match the replaced lines (`add_pr_comment(end_line=...)`) or Apply edits the wrong place. Suggestions are only in this flow, not the pipeline's own review step. See KN-45.
