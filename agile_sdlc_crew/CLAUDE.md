...
- **PR review contribution** (`pr_review.py`, `POST /api/pr-review`, board 🔍 on started WIs; `CREW_PR_REVIEW` on): advisory review of a human-developed PR — resolves the active PR from WI relations, pre-fetches changed files, runs `code_reviewer` once, posts a summary + inline blocker/major comments on the PR and a WI comment. Never changes code, votes, or WI state. Jobs carry `job_kind=pr_review`; retro/sprint-plan/daily queries filter `job_kind=pipeline`. See KN-45.
