...
- **Agent skills** (`skills/` package, `CREW_AGENT_SKILLS` on): CrewAI's native Agent Skills (`Agent(skills=[...])`, `SKILL.md` frontmatter, progressive disclosure) — **not** the `knowledge/` mechanism, which injects a whole file every call. `skill_kwargs("code-review")` passes **pre-loaded Skill objects**, never a directory path: CrewAI treats a `Path` as a *search dir* and would attach every skill to every agent. Skills load at RESOURCES level so the body plus a `references/` catalog reach the system prompt. The reviewer can't open those files itself (it runs on claude_cli, where the CrewAI tool bridge is broken), so `skills.review_context(repo)` resolves the repo language via `knowledge.detect_repo_type` and injects the ONE matching reference + `sonarqube.md` — both `flow._review_skill_context` (step8) and `pr_review.build_pr_context` call it, so pipeline and human-PR reviews judge by the same yardstick. The `code-review` skill binds rejection to three gates — **missing / broken / extra** vs the work item — everything else is `minor`. Sonar follows **Clean as You Code**: only violations on lines the PR touched block (repos run `sonar.qualitygate.wait=true`, so the gate fails their build); pre-existing debt is out of scope, and new-code coverage is a `minor` warning because `CREW_REQUIRE_TESTS` owns that. See KN-48.
